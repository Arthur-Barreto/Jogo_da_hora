...
class Idle(pg.sprite.Sprite):
    #Construtor da Classe
    def __init__(self,centerx,centery,bottom,assets):
        pg.sprite.Sprite.__init__(self)
        self.player_idle =  assets["player"]
        self.frame = 0
        self.image = self.player_idle[self.frame]
        self.rect = self.image.get_rect()
    ...
    def update(self):
        ...
        if elapsed_ticks > self.frame_ticks:
            ...
            self.last_update = now
            self.frame +=1
            if self.frame == len(self.player_idle):
                self.kill()
            else:
                center = self.rect.center
                self.image = self.player_idle[self.frame]
